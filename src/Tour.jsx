...
export default function Tour({ setView, setShowSettings, setSettingsTab }) {
  ...
  if (isModal || !targetRect) {
    ...
    const pos = computeTooltipPos(expanded);
    tipStyle = { top: pos.top, left: pos.left, width: pos.width };
  }

  // Spotlight rect (if any)
  ...
  const dimStyle = (isModal || !targetRect) ? {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 99996, pointerEvents: 'none' }}>
      ...
      <div
        ...
        {step.pre && (
          <div style={{ fontSize: 10, letterSpacing: '0.3em', color: '#C8942A', textTransform: 'uppercase', marginBottom: 8 }}>{step.pre}</div>
        )}
        <div style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.3, marginBottom: 10 }}>{step.title}</div>
        ...
        <div style={{ position: 'absolute', top: 12, right: 14, fontSize: 11, color: '#666' }}>
          {stepIdx + 1} / {total}
        </div>
      </div>
    </div>
